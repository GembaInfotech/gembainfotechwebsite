...
import img13 from '../../assets/Agrachain.png';
...
function PartnerCompany() {
    return (
      ...
      <div className="grid grid-cols-1  md:grid-cols-2 lg:grid-cols-3 gap-1 my-14 mx-2 md:mx-24">
        {images.map((image) => (
          <div key={image.id} className="relative">
            <img
              src={image.src}
              alt={image.title}
              className="w-[50vh] h-[20vh] md:w-[60vh] md:h-[30vh] bg-white shadow-lg p-8"
            />
          </div>
        ))}
      </div>
